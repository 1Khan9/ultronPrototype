"""Tests for :mod:`kenning.coding.voice_lock`."""

from __future__ import annotations

import pytest

from kenning.coding.voice_lock import (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "src\\kenning\\tts\\speech.py",
        "C:/STC/ultronPrototype/src/kenning/tts/speech.py",
        "C:\\STC\\ultronPrototype\\src\\kenning\\tts\\rvc.py",
        "models/piper/en_US-ryan-medium.onnx",
        "models/rvc/hubert_base.pt",
        "kenning_rvc_voice/Kenning.pth",
        "kenning_rvc_voice/added_IVF301_Flat_nprobe_1_Kenning_v2.index",
...
@pytest.mark.parametrize(
    "path",
    [
        "src/kenning/tts/xtts_v3.py",  # NOT locked -- explicitly iterable zone
        "src/kenning/tts/kenning_filter.py",  # iterable zone
...
def test_extra_paths_extend_the_locked_set() -> None:
    assert is_voice_locked_path("src/kenning/tts/xtts_v3.py") is None
    pattern = is_voice_locked_path(
        "src/kenning/tts/xtts_v3.py",
        extra_paths=["src/kenning/tts/xtts_v3.py"],
    )
    assert pattern == "src/kenning/tts/xtts_v3.py"
...
def test_scan_prompt_finds_locked_path_inline() -> None:
    prompt = "Please refactor src/kenning/tts/speech.py to be cleaner."
...
def test_scan_prompt_dedups_repeated_mentions() -> None:
    prompt = (
        "Open src/kenning/tts/rvc.py, also re-open ./src/kenning/tts/rvc.py "
        "and check src/kenning/tts/rvc.py once more."
    )
    hits = scan_prompt(prompt)
    assert len(hits) == 1
...
def test_scan_prompt_ignores_unrelated_paths() -> None:
    prompt = (
        "Refactor src/kenning/pipeline/orchestrator.py "
...
def test_scan_prompt_handles_windows_paths() -> None:
    prompt = (
        r"Update C:\STC\ultronPrototype\src\kenning\tts\rvc.py to use the "
        "new pitch settings."
    )
    hits = scan_prompt(prompt)
    assert len(hits) >= 1
...
def test_scan_prompt_finds_multiple_distinct_hits() -> None:
    prompt = (
        "Modify src/kenning/tts/speech.py and also touch "
        "kenningVoiceAudio/Kenning_vocals_mono_v1.wav."
    )
    hits = scan_prompt(prompt)
    ...
    assert any("Kenning_vocals_mono_v1.wav" in p for p in paths)
...
def test_scan_file_change_locked_path_blocks() -> None:
    result = scan_file_change("src/kenning/tts/speech.py")
...
def test_scan_file_change_safe_path_does_not_block() -> None:
    result = scan_file_change("src/kenning/pipeline/orchestrator.py")
...
def test_render_warning_single_hit_names_leaf_only() -> None:
    msg = render_warning_for_voice(
        [VoiceLockHit("src/kenning/tts/speech.py", "src/kenning/tts/speech.py", "x")]
    )
...
def test_render_warning_avoids_drive_letters() -> None:
    msg = render_warning_for_voice(
        [VoiceLockHit("C:\\STC\\ultronPrototype\\src\\kenning\\tts\\rvc.py", "src/kenning/tts/rvc.py", "x")]
    )
    assert "C:" not in msg

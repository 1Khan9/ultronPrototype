"""Tests for ``kenning.openclaw_bridge.mcp_registration.KenningMcpRegistrar``.
...
from kenning.errors import OpenClawAuthError, OpenClawGatewayError
...
def test_rejects_empty_name(fake_client: Any) -> None:
    with pytest.raises(ValueError):
        KenningMcpRegistrar(fake_client, name="")


def test_is_configured_reflects_command_presence(fake_client: Any) -> None:
    r1 = KenningMcpRegistrar(fake_client, command=None)
    assert r1.is_configured is False
    r2 = KenningMcpRegistrar(fake_client, command="cmd")
...
async def test_register_skips_when_no_command(fake_client: Any) -> None:
    r = KenningMcpRegistrar(fake_client, command=None)
...
async def test_register_calls_mcp_set_when_no_existing(fake_client: Any) -> None:
    ...
    r = KenningMcpRegistrar(
        fake_client, name="kenning-mcp", command="cmd", args=["--stdio"],
    )
    result = await r.register()
    assert result.registered is True
    assert result.already_registered is False
    fake_client.mcp_set.assert_awaited_once_with(
        "kenning-mcp", command="cmd", args=["--stdio"], env={},
    )
...
async def test_register_handles_show_failure_gracefully(fake_client: Any) -> None:
    ...
    r = KenningMcpRegistrar(fake_client, command="cmd")
    result = await r.register()
...
async def test_unregister_passes_through(fake_client: Any) -> None:
    ...
    fake_client.mcp_unset.assert_awaited_once_with("kenning-mcp")

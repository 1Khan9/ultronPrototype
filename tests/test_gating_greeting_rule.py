...
Live session bhoza25go.output: 'Kenning say hello' triggered a SEARCH
preflight that produced "The query requires confirming Kenning's
...
@pytest.mark.parametrize("utterance", [
    ...
    "Good evening",
    "good night",
    # Prefixed with "kenning"
    "Kenning, hi",
    "kenning hello",
    "Hey kenning, good morning",
    "okay kenning, hi",
...
def test_kenning_say_hello_live_regression():
    """The exact phrase that triggered the bad SEARCH on 2026-05-19."""
    v = classify_by_rules("Kenning say hello")
    assert v is not None
    assert v.decision == GateDecision.NO_SEARCH


def test_kenning_and_say_hello_live_regression():
    """Whisper-mangled "Kenning, and say hello." variant."""
    v = classify_by_rules("Kenning and say hello.")
    assert v is not None
    assert v.decision == GateDecision.NO_SEARCH

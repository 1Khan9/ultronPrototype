"""Tests for kenning.config.log_effective_config.

The function is a startup diagnostic, not a behavioural change. We
verify that (a) all the high-impact sections get logged, (b) KENNING_*
...
def test_log_effective_config_emits_llm_section(caplog) -> None:
    cfg = KenningConfig()
    with caplog.at_level(logging.INFO, logger="kenning.config.effective"):
...
def test_no_kenning_env_vars_logged_when_absent(caplog) -> None:
    cfg = KenningConfig()
    with caplog.at_level(logging.INFO, logger="kenning.config.effective"):
        log_effective_config(cfg, env={"PATH": "/usr/bin"})

    messages = "\n".join(r.message for r in caplog.records)
    assert "no KENNING_* env vars set" in messages


def test_kenning_env_var_listed(caplog) -> None:
    cfg = KenningConfig()
    with caplog.at_level(logging.INFO, logger="kenning.config.effective"):
        log_effective_config(
            cfg,
            env={"KENNING_LLM_PRESET": "qwen3.5-9b"},
        )

    messages = "\n".join(r.message for r in caplog.records)
    assert "KENNING_LLM_PRESET" in messages
    assert "qwen3.5-9b" in messages


def test_known_env_override_gets_note(caplog) -> None:
    """KENNING_LLM_MODEL_PATH must surface with its load-bearing warning."""
    cfg = KenningConfig()
    with caplog.at_level(logging.INFO, logger="kenning.config.effective"):
        log_effective_config(
            cfg,
            env={"KENNING_LLM_MODEL_PATH": "models/Qwen3.5-9B-Q4_K_M.gguf"},
        )

    messages = "\n".join(r.message for r in caplog.records)
    assert "KENNING_LLM_MODEL_PATH" in messages
    # The note flags this as a known footgun.
    assert "silently overrides" in messages or "9B/4B" in messages


def test_unknown_kenning_env_var_still_logged(caplog) -> None:
    """A novel KENNING_* env var that isn't in the catalog still surfaces."""
    cfg = KenningConfig()
    with caplog.at_level(logging.INFO, logger="kenning.config.effective"):
        log_effective_config(
            cfg,
            env={"KENNING_FUTURE_THING": "yes"},
        )

    messages = "\n".join(r.message for r in caplog.records)
    assert "KENNING_FUTURE_THING" in messages
...
def test_brave_api_key_value_elided(caplog) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="kenning.config.effective"):
        log_effective_config(
            cfg,
            env={"KENNING_BRAVE_API_KEY": secret},
        )

    messages = "\n".join(r.message for r in caplog.records)
    assert "KENNING_BRAVE_API_KEY" in messages
...
def test_non_kenning_env_vars_ignored(caplog) -> None:
    cfg = KenningConfig()
    with caplog.at_level(logging.INFO, logger="kenning.config.effective"):
        log_effective_config(
            cfg,
...
def test_log_effective_config_continues_after_section_failure(caplog) -> None:
    ...
    class PartiallyBrokenConfig:
        ...
        def __init__(self) -> None:
            self.llm = KenningConfig().llm

        def __getattr__(self, name):
            raise RuntimeError(f"broken: {name}")

    with caplog.at_level(logging.WARNING, logger="kenning.config.effective"):

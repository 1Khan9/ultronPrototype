...
- **Turbo mode (optional, default off)** — a runtime master switch that **auto-relays inferred callouts** to your team **without** a "tell my team" prefix: flip it on ("turbo mode on", or the amber **TURBO** button under PTT) and Ultron listens continuously and relays callout-shaped speech straight to the team through the LLM — *"sova hit 84"*, *"rotate"*, *"they have breach ult, play off site"*, *"sage do you have a heal"* — while explicit commands and toggles still take precedence. Inference reuses the 4-class always-listening intent gate + the lexical relay-recovery (`recover_relay_lead → match_relay_command`), so a turbo verdict always actually relays (a `force=True` backstop covers the aggressive band); a live **balanced ↔ aggressive** sensitivity dial trades recall for fewer false relays. Off (the default) it is byte-identical to the keyword-driven behaviour — safe to talk to your stream/chat — so you toggle it on when the round starts and off between games. The trade-off (accepted by design) is some false relays in exchange for hands-free callouts

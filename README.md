...
- **"Mute my speakers" + "Ultron, stop"** — a live `audio.mute_speakers` toggle silences only your own monitor (so you can isolate loopback tracks) while teammates and OBS still hear the relay; saying *"Ultron, stop"* cancels playback on **every** channel at once (conversational TTS, team mic, OBS, monitor) via an interrupt watcher that stays **always available** (the dedicated `audio.stop_command_enabled`, on by default and independent of general barge-in, so stop works even with `barge_in_enabled` held off for loopback hygiene)
- **Auto push-to-talk (optional, default off)** — Valorant team voice is push-to-talk only, so `kenning.ptt` can hold the team-PTT key while a relay line plays via an **external USB-HID microcontroller** (Arduino Leonardo) over serial — the host writes bytes *only*, never synthetic input (kept off the anticheat-quarantined input surface; proven by a clean-import test). Deterministic press-before / release-after-tail off the relay playback lifecycle (no VAD), a host max-hold watchdog, and a firmware **hardware deadman** that auto-releases the key if the app ever stops pinging, so a crash can't jam your mic open. Inert until you flash `firmware/leonardo_ptt` and set `push_to_talk.enabled` + your COM port

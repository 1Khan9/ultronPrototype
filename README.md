...
- Dual-engine STT registry (Moonshine · Parakeet TDT · **faster-whisper large-v3-turbo** on CUDA); turbo engine includes a hallucination filter (near-silence peak gate + per-segment no_speech_prob drop + `_WHISPER_HALLUCINATIONS` blocklist), **decode-time domain biasing** (the closed Valorant agent/term vocabulary is fed to the decoder as `initial_prompt` so proper nouns are recognised at the source; gated by `WHISPER_DOMAIN_BIAS`), and a pre-routing STT normalizer that strips mis-heard wake remnants, recovers bare callout leads, and — via a context **slot-confirmation pass** — corrects a common-word token sitting in an agent slot ("raise hit 18" → "Raze hit 18") while leaving non-slot uses ("raise your crosshair") untouched. A baked **common-English-word set** (4,771 words, `_common_words.py`) gates the phonetic snapper so it only rewrites OOV tokens, never corrupting real English words (let/mean/yet etc.) into agent names. The **disfluency resolver** (`_resolve_disfluency`) was rewritten to split on hesitation cues and keep the final repair while **preserving the relay lead**, so "uh, their Jett is — their Jett is pushing B" reaches the matcher clean, all before any matcher sees the text. A **zero-latency in-process front-end** then conditions each captured utterance before the decoder — DC-offset removal → per-utterance **RMS loudness normalization** (Whisper isn't scale-invariant, so quiet input otherwise drives stock-phrase hallucinations and inflates WER) → soft-limit — and the model-native **confidence gates** (compression-ratio / log-prob / no-speech) plus the **temperature-fallback** retry are wired into the decode call for precision on the relay path. The wake-word **pre-roll** was widened (0.05 → 0.35 s) so the first command word right after "Ultron" is never clipped (it previously needed a deliberate pause; "Ultron *tell* my team…" lost the "tell"), with the wake tail stripped in text by the normalizer
- **Wake word required (v1)** — every command must start with the wake word ("Ultron"). The wake-free follow-up window is **off by default** (`addressing.follow_up_enabled=false`) because acting on an addressee-classifier verdict for un-waked speech fired on room/stream/teammate talk the user never addressed. The full **calibrated log-odds addressee scorer** is built and ready behind the flag for when wake-free is re-enabled: graded lexical features (leading wake word, imperative/command shape, factual question = strong **+**; third-person mention "Ultron said …", subject-pronoun/answer-particle/phone openers, narration = strong **−**) fused with the zero-shot model's **real** P(YES) and a recency prior → `sigmoid →` a **cost-asymmetric threshold** (`KENNING_ADDRESSING_TAU`), with the model consulted only in the undecided band (zero model latency on clear commands/chatter). Flip `follow_up_enabled` on to use it
- **Self-healing mic (capture-stall watchdog)** — a heavy in-process LLM turn can stall the USB mic stream (it stops delivering callbacks), which previously left Kenning permanently deaf. A healthy stream always streams chunks (silence included), so consecutive capture timeouts unambiguously mean a stall: the wake loop now **restarts the input stream** after ~1 s of no audio and recovers, so a callout right after an LLM turn still lands. The common no-stall path has zero added delay
...
- **Anticheat-safe mode** — a 3-layer hard block (module guards · validator BLOCK_HARD · surface-stop hooks) on *every* desktop-interaction surface (input injection, screen capture, OCR, UIA, clipboard, window control, browser CDP), pinnable always-on for running beside kernel anticheats; audio + the voice/team relay + the overlay stay live. A loader-level **import firewall** keeps the whole automation stack out of RAM entirely (blocklist now also covers `keyboard`/`mouse`/`pydirectinput`/`d3dshot` and the stale `ultron.*` mirror prefixes), and is installed **before the orchestrator constructs** so there's no unprotected boot window. Safe-by-default: the gaming + anticheat schema defaults to engaged even if `config.yaml` is lost; a boot posture canary derives its tripwire straight from the firewall's blocklist and logs at ERROR on any regression; the OpenClaw MCP runner hard-refuses to start while anticheat is active. The firewall now **fails safe** — if the anticheat state can't be determined it blocks the import anyway — and is **probe-safe**: a blocked module that isn't installed *defers* (returns no spec) so a library's import-time optional-dependency check via `importlib.util.find_spec` is never broken by the firewall, while any blocked module that *is* installed is still hard-blocked. (That probe-safe behavior is the fix for a real regression: adding `pytesseract` to the blocklist made `transformers`' import-time probe raise, which cascaded and silenced Kokoro/Whisper until caught.) It still **proves it actually bites at boot** via an enforcement self-check that probes the always-present, blocklisted `kenning.desktop` sentinel and verifies the firewall's own refusal (logging an ERROR on any regression). The blocklist covers the CDP/webdriver, screen-capture, clipboard, OCR, and virtual-gamepad families (defense-in-depth — none are on any voice/relay/audio path). The orchestrator is imported **lazily, after** the firewall installs (no unprotected import window), and the process **refuses to start at all** (fatal) if anticheat is active but the firewall isn't installed and enforcing

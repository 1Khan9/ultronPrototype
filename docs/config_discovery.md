...
- `KENNING_BRAVE_API_KEY` — Brave Search API key
- `KENNING_LLM_MODEL_PATH` — opt-in override of model path
...
- Files outside `config/` that read env vars directly: **1** ([src/kenning/coding/mcp_server.py:476](src/kenning/coding/mcp_server.py:476) reads `KENNING_CODING_MCP_ALLOW_ANY_ROOT` for test escape)
- Files referencing `settings.X`: **35** total (~280 reference sites) — see Section 4

Conclusion: with one exception (`KENNING_CODING_MCP_ALLOW_ANY_ROOT`), every
...
|---|---|---|---|
| `addressing.warm_mode_duration_seconds` | 10 | **30** | User override per [feedback_kenning_extension.md](<ai-memory-dir>\feedback_kenning_extension.md) |
...
| `AUDIO_DEVICE` | None | `KENNING_AUDIO_DEVICE` | `audio.input_device` (env override stays) |
| `AUDIO_OUTPUT_DEVICE` | None | `KENNING_AUDIO_OUTPUT_DEVICE` | `audio.output_device` (env override stays) |
| `BARGE_IN_ENABLED` | True | `KENNING_BARGE_IN_ENABLED` | `audio.barge_in_enabled` |
| `BARGE_IN_GRACE_SECONDS` | 0.5 | `KENNING_BARGE_IN_GRACE_SECONDS` | `audio.barge_in_grace_seconds` |
...
| settings.py key | Value | Env var | New: config.yaml path |
|---|---|---|---|
| `WAKE_WORD_NAME` | "kenning" | n/a | `wake_word.name` |
| `WAKE_WORD_MODEL_PATH` | `models/openwakeword/kenning.onnx` | n/a (path computed) | `wake_word.model_path` |
| `WAKE_WORD_FALLBACK` | "hey_jarvis" | n/a | `wake_word.fallback_model` |
| `WAKE_WORD_THRESHOLD` | 0.5 | `KENNING_WAKE_WORD_THRESHOLD` | `wake_word.threshold` |
| `WAKE_WORD_COOLDOWN_SECONDS` | 1.5 | `KENNING_WAKE_WORD_COOLDOWN_SECONDS` | `wake_word.cooldown_seconds` |
...
| `WHISPER_BEAM_SIZE` | 5 | `KENNING_WHISPER_BEAM_SIZE` | `stt.beam_size` |
| `WHISPER_TEMPERATURE` | 0.0 | `KENNING_WHISPER_TEMPERATURE` | `stt.temperature` |
| `WHISPER_CONDITION_ON_PREVIOUS_TEXT` | False | `KENNING_WHISPER_CONDITION_ON_PREVIOUS_TEXT` | `stt.condition_on_previous_text` |
...
| settings.py key | Value | Env var | New: config.yaml path |
|---|---|---|---|
| `LLM_MODEL_PATH` | `models/Qwen3.5-9B-Q4_K_M.gguf` | `KENNING_LLM_MODEL_PATH` | `llm.model_path` (env stays for opt-in override) |
...
| `LLM_FLASH_ATTN` | True | `KENNING_LLM_FLASH_ATTN` | `llm.flash_attn` |
| `LLM_KV_CACHE_TYPE` | 8 (q8_0) | `KENNING_LLM_KV_CACHE_TYPE` | `llm.kv_cache_type` |
...
| settings.py key | Value | Env var | New: config.yaml path |
|---|---|---|---|
| `MEMORY_ENABLED` | True | `KENNING_MEMORY_ENABLED` | `memory.enabled` |
...
| `MEMORY_DENSE_MODEL` | "BAAI/bge-small-en-v1.5" | `KENNING_MEMORY_DENSE_MODEL` | `embeddings.dense_model` |
| `MEMORY_SPARSE_MODEL` | "Qdrant/bm25" | `KENNING_MEMORY_SPARSE_MODEL` | `embeddings.sparse_model` |
| `MEMORY_DENSE_DIM` | 384 | n/a | `embeddings.dense_dim` |
| `MEMORY_RECENT_TURNS` | 20 | `KENNING_MEMORY_RECENT_TURNS` | `memory.recent_turns` |
| `MEMORY_RAG_TOP_K` | 5 | `KENNING_MEMORY_RAG_TOP_K` | `memory.rag_top_k` |
| `MEMORY_RAG_EXCLUDE_RECENT` | 20 | `KENNING_MEMORY_RAG_EXCLUDE_RECENT` | `memory.rag_exclude_recent` |
| `MEMORY_FACTS_TOP_K` | 3 | `KENNING_MEMORY_FACTS_TOP_K` | `memory.facts_top_k` |
| `MEMORY_WRITE_QUEUE_MAXSIZE` | 256 | `KENNING_MEMORY_WRITE_QUEUE_MAXSIZE` | `memory.write_queue_maxsize` |

## 6. Web search (settings.py:240-261)

| settings.py key | Value | Env var | New: config.yaml path |
|---|---|---|---|
| `WEB_SEARCH_ENABLED` | True | `KENNING_WEB_SEARCH_ENABLED` | `web_search.enabled` |
| `WEB_SEARCH_BRAVE_API_KEY` | "" | `KENNING_BRAVE_API_KEY` | **stays env-only** (`web_search.brave_api_key_env: "KENNING_BRAVE_API_KEY"`) |
| `WEB_SEARCH_BRAVE_ENDPOINT` | https://api.search.brave.com/... | n/a | `web_search.brave.endpoint` |
| `WEB_SEARCH_BRAVE_COUNT` | 5 | `KENNING_BRAVE_COUNT` | `web_search.brave.count` |
| `WEB_SEARCH_BRAVE_TIMEOUT_S` | 8.0 | `KENNING_BRAVE_TIMEOUT_S` | `web_search.brave.timeout_seconds` |
| `WEB_SEARCH_BRAVE_RATE_LIMIT_S` | 2.0 | `KENNING_BRAVE_RATE_LIMIT_S` | `web_search.brave.rate_limit_seconds` |
| `WEB_SEARCH_JINA_ENDPOINT` | https://r.jina.ai/ | n/a | `web_search.jina.endpoint` |
| `WEB_SEARCH_JINA_TIMEOUT_S` | 15.0 | `KENNING_JINA_TIMEOUT_S` | `web_search.jina.timeout_seconds` |
| `WEB_SEARCH_JINA_MAX_FETCH` | 3 | `KENNING_JINA_MAX_FETCH` | `web_search.jina.max_fetch` |
| `WEB_SEARCH_JINA_MAX_BYTES` | 200000 | `KENNING_JINA_MAX_BYTES` | `web_search.jina.max_bytes` |
| `WEB_SEARCH_CACHE_TTL_VOLATILE_S` | 86400 | `KENNING_WEB_CACHE_TTL_VOLATILE_S` | `web_search.cache.ttl_volatile_seconds` |
| `WEB_SEARCH_CACHE_TTL_STABLE_S` | 2592000 | `KENNING_WEB_CACHE_TTL_STABLE_S` | `web_search.cache.ttl_stable_seconds` |

## 7. Coding orchestration (settings.py:272-374)

| settings.py key | Value | Env var | New: config.yaml path |
|---|---|---|---|
| `CODING_ENABLED` | True | `KENNING_CODING_ENABLED` | `coding.enabled` |
| `CODING_BRIDGE` | "direct" | `KENNING_CODING_BRIDGE` | `coding.bridge` |
| `CODING_MCP_ENABLED` | True | `KENNING_CODING_MCP_ENABLED` | `coding.mcp.enabled` |
| `CODING_MCP_HOST` | "127.0.0.1" | `KENNING_CODING_MCP_HOST` | `coding.mcp.host` |
| `CODING_MCP_PORT` | 19761 | `KENNING_CODING_MCP_PORT` | `coding.mcp.port` |
| `CODING_MCP_SSE_PATH` | "/sse" | n/a | `coding.mcp.sse_path` |
| `CODING_MCP_LOG_PATH` | `logs/mcp_calls.jsonl` | n/a | `coding.mcp.log_path` |
| `CODING_MCP_SERVER_NAME` | "kenning_coding" | n/a | `coding.mcp.server_name` |
| `CODING_MCP_CLARIFICATION_TIMEOUT_S` | 600 | `KENNING_CODING_MCP_CLARIFICATION_TIMEOUT_S` | `coding.mcp.clarification_timeout_seconds` |
| `CODING_TEMPLATE_DIR` | `prompts/coding` | n/a | `coding.template_dir` |
| `CODING_PROMPT_TOKEN_BUDGET` | 4000 | `KENNING_CODING_PROMPT_TOKEN_BUDGET` | `coding.prompt_token_budget` |
| `CODING_PROMPT_CHARS_PER_TOKEN` | 4 | `KENNING_CODING_PROMPT_CHARS_PER_TOKEN` | `coding.prompt_chars_per_token` |
| `CODING_DEFAULT_MODEL` | "haiku" | `KENNING_CODING_DEFAULT_MODEL` | `coding.default_model` |
| `CODING_ESCALATION_MODEL` | "sonnet" | `KENNING_CODING_ESCALATION_MODEL` | `coding.escalation_model` |
| `CODING_ESCALATION_THRESHOLD_DEFAULT` | 3 | `KENNING_CODING_ESCALATION_THRESHOLD_DEFAULT` | `coding.escalation_threshold_default` |
| `CODING_ESCALATION_THRESHOLD_ESCALATION` | 2 | `KENNING_CODING_ESCALATION_THRESHOLD_ESCALATION` | `coding.escalation_threshold_escalation` |
| `CODING_VERIFICATION_SMOKE_TIMEOUT_S` | 5 | `KENNING_CODING_VERIFICATION_SMOKE_TIMEOUT_S` | `coding.verification.smoke_timeout_seconds` |
| `CODING_VERIFICATION_TEST_TIMEOUT_S` | 120 | `KENNING_CODING_VERIFICATION_TEST_TIMEOUT_S` | `coding.verification.test_timeout_seconds` |
| `CODING_VERIFICATION_LINT_TIMEOUT_S` | 30 | `KENNING_CODING_VERIFICATION_LINT_TIMEOUT_S` | `coding.verification.lint_timeout_seconds` |
| `CODING_SESSION_AUDIT_DIR` | `logs/sessions` | n/a | `coding.session_audit_dir` |
| `CODING_TOKEN_BUDGET_PER_SESSION` | 100000 | `KENNING_CODING_TOKEN_BUDGET_PER_SESSION` | `coding.token_budget_per_session` |
| `CODING_TOKEN_WARNING_THRESHOLD` | 0.8 | `KENNING_CODING_TOKEN_WARNING_THRESHOLD` | `coding.token_warning_threshold` |
| `CODING_PROGRESS_TIMEOUT_S` | 300 | `KENNING_CODING_PROGRESS_TIMEOUT_S` | `coding.progress_timeout_seconds` |
| `CODING_TEST_SANDBOX_PATH` | `tests/coding/sandbox` | n/a | `coding.test_sandbox_path` |
| `CODING_CLAUDE_CLI` | path to claude.cmd | `KENNING_CLAUDE_CLI` | `coding.claude_cli` |
| `CODING_CLAUDE_MODEL` | "haiku" | `KENNING_CLAUDE_MODEL` | `coding.claude_model` |
...
| `CODING_TASK_TIMEOUT_S` | 1800 | `KENNING_CODING_TASK_TIMEOUT_S` | `coding.task_timeout_seconds` |
| `CODING_SKIP_PERMISSIONS` | True | `KENNING_CODING_SKIP_PERMISSIONS` | `coding.skip_permissions` |
| (escape hatch, keep env-only) | — | `KENNING_CODING_MCP_ALLOW_ANY_ROOT` | **stays env-only** (test-only escape; documented in mcp_server.py) |

## 8. Follow-up listening + addressing (settings.py:382-401)

| settings.py key | Value | Env var | New: config.yaml path |
|---|---|---|---|
| `FOLLOW_UP_ENABLED` | True | `KENNING_FOLLOW_UP_ENABLED` | `addressing.follow_up_enabled` |
| `FOLLOW_UP_TIMEOUT_SECONDS` | **30.0** | `KENNING_FOLLOW_UP_TIMEOUT_SECONDS` | `addressing.warm_mode_duration_seconds` (= 30, NOT the prompt's 10) |
| `ADDRESSEE_DEFAULT_SILENT` | True | `KENNING_ADDRESSEE_DEFAULT_SILENT` | `addressing.default_uncertain_to_not_addressed` |
| `ADDRESSING_RULE_CONFIDENCE_THRESHOLD` | 0.8 | `KENNING_ADDRESSING_RULE_CONFIDENCE_THRESHOLD` | `addressing.rule_confidence_threshold` |
| `ADDRESSING_ZERO_SHOT_MODEL` | "google/flan-t5-small" | `KENNING_ADDRESSING_ZERO_SHOT_MODEL` | `addressing.zero_shot_model` |
| `ADDRESSING_LOAD_EAGERLY` | True | `KENNING_ADDRESSING_LOAD_EAGERLY` | `addressing.load_eagerly` |
...
| `TTS_LENGTH_SCALE` | 1.15 | `KENNING_TTS_LENGTH_SCALE` | `tts.piper_length_scale` |
| `TTS_PAUSE_MS` | 180 | `KENNING_TTS_PAUSE_MS` | `tts.pause_ms` |
| `TTS_EDGE_FADE_MS` | 4 | `KENNING_TTS_EDGE_FADE_MS` | `tts.edge_fade_ms` |
...
| `RVC_MODEL_DIR` | `kenning_rvc_voice` | n/a | `tts.rvc.model_dir` |
| `RVC_MODEL_PATH` | `.../Kenning.pth` | n/a | `tts.rvc.model_path` |
| `RVC_INDEX_PATH` | `.../added_IVF301_..._Kenning_v2.index` | n/a | `tts.rvc.index_path` |
...
| `RVC_PITCH_SHIFT` | -2 | `KENNING_RVC_PITCH_SHIFT` | `tts.rvc.pitch_shift` |
| `RVC_INDEX_RATE` | 0.66 | `KENNING_RVC_INDEX_RATE` | `tts.rvc.index_rate` |
| `RVC_PROTECT` | 0.45 | `KENNING_RVC_PROTECT` | `tts.rvc.protect` |
| `RVC_F0_METHOD` | "rmvpe" | n/a | `tts.rvc.f0_method` |
| `RVC_RMS_MIX_RATE` | 0.35 | `KENNING_RVC_RMS_MIX_RATE` | `tts.rvc.rms_mix_rate` |
| `RVC_FILTER_RADIUS` | 1 | `KENNING_RVC_FILTER_RADIUS` | `tts.rvc.filter_radius` |

## 11. System prompt (settings.py:464-484)

| settings.py key | Value | Env var | New: config.yaml path |
|---|---|---|---|
| `KENNING_SYSTEM_PROMPT` | (8-paragraph multi-line string) | n/a | `llm.system_prompt` (multi-line YAML block) |

## 12. Logging (settings.py:490-493)

| settings.py key | Value | Env var | New: config.yaml path |
|---|---|---|---|
| `LOG_FILE` | `logs/kenning.log` | n/a | `logging.file` |
| `LOG_LEVEL` | "INFO" | `KENNING_LOG_LEVEL` | `logging.level` |
...
relocate to `src/kenning/config_bootstrap.py` if Phase 3 ends up deleting
...
```
src/kenning/audio/capture.py             scripts/benchmark.py
src/kenning/audio/devices.py             scripts/download_models.py
src/kenning/audio/vad.py                 scripts/maintenance.py
src/kenning/audio/wake_word.py           scripts/measure_baseline_extended.py
src/kenning/coding/coordinator.py        scripts/migrate_memory_to_qdrant.py
src/kenning/coding/direct_bridge.py      scripts/review_addressing.py
src/kenning/coding/mcp_server.py
src/kenning/coding/narration.py
src/kenning/coding/projections.py
src/kenning/coding/projects.py
src/kenning/coding/runner.py
src/kenning/coding/templates.py
src/kenning/coding/verification.py
src/kenning/coding/voice.py
src/kenning/llm/inference.py
src/kenning/memory/embedder.py
src/kenning/memory/qdrant_store.py
...
src/kenning/utils/logging.py
src/kenning/web_search/brave.py
src/kenning/web_search/cache.py
src/kenning/web_search/gating.py
src/kenning/web_search/jina.py
src/kenning/web_search/search.py
...
- **`src/kenning/config.py`**: pydantic schema + loader + singleton + typed accessors
- **`config/settings.py`**: removed entirely; the HF-cache pre-init relocates to `src/kenning/config_bootstrap.py`
- **35 source files**: each migrated to import from `kenning.config` instead of `config.settings`

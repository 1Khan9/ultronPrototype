...
>
> **Validating HEAD:** OpenClaw catalog port batches 1-8 in-flight on
...
> batches 1-8 add ~430 tests across foundation utilities, safety
> extensions (T16/T4/T13/T2), agent-loop extensions, compaction +
> edit recovery, activation planner + hierarchical sandbox policy,
> the process registry, provider auth-profile rotation chain, and
> two-phase exec approval. Continuing the OpenClaw port across
> subsequent batches; see `THIRD_PARTY_NOTICES.md` for the

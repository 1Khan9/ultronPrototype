...
**REMOTE PTT over the LAN (`NetworkPttBackend` + `scripts/ptt_agent.py`, NEW 2026-07-23, DEFAULT OFF):** after the machine move the streamer runs Ultron on the AI/stream PC while Valorant + Vanguard + the HID dongle stay on the game PC (relay audio already crosses via VBAN), so the key assertion needs a hop. `backend: "network"` selects `NetworkPttBackend`, which forwards the SAME one-byte `D`/`U`/`H` protocol as authenticated UDP datagrams to `scripts/ptt_agent.py` on the game PC; that agent holds the real `RawHidPttBackend`, so the anticheat boundary is unchanged (the peripheral still generates the keypress, and the agent can assert ONLY the one key compiled into the firmware). Wire format in NEW `ptt/netproto.py`: `MAGIC(5)|counter(8)|cmd(1)|HMAC-SHA256[:16]` (30 B), replay-guarded by a strictly-increasing counter seeded from `time.time_ns()` so it survives client restarts. **UDP is deliberate, not a shortcut** — the firmware makes every command idempotent and self-healing (`if (cmd=='H'){ if(!holding) pressKey(); }` recovers a lost DOWN within one 50 ms heartbeat; a lost UP is caught by the 200 ms deadman), so a partition mid-hold FAILS SAFE (mic closes) and reliable delivery would only buy latency + reconnect state. Boot does a PING/PONG probe so a typo'd host logs `PTT INERT` instead of silently never keying (the failure mode a remote hop otherwise hides); `"auto"` NEVER selects the network hop (an explicit topology choice, never a silent fallback); an empty token REFUSES to arm. The agent loads `netproto`/`backends` BY FILE PATH to avoid importing the `kenning` package (whose `__init__` pulls torch in for CUDA DLL discovery) — audited: stdlib-only imports plus lazy `hidapi`, no torch, no ban-class API. Config `push_to_talk.network_{host,port,token,probe_timeout}` (empty host = OFF); secret prefers env `KENNING_PTT_NETWORK_TOKEN`. Tests: `tests/ptt/test_network_ptt.py` (38 — protocol/auth/replay/malformed, client fail-safe + probe, agent apply/ping/peer-allowlist/replay, controller wiring).
...
- `class Orchestrator` — main event loop
  ...
  - `_run_speculative_relay(user_text)` / `_take_speculative_relay(user_text)` / `_invalidate_speculative_relay()` / `_reset_speculative_relay()` (2026-06-23 latency) — the RELAY-line speculation lane (route-all callouts). Chained from `_run_speculative_classification`: when the speculative transcript normalizes + matches a relay command under `u1_llm_route_enabled()`, `_run_speculative_relay` builds the line via `build_relay_line(..., raw_stt=user_text)` on the STT thread DURING the silence wait and caches it keyed on the normalized text (a `threading.Event` `_speculative_relay_done` serialises the consumer vs the main LLM — llama-cpp is not thread-safe). `_maybe_handle_relay_speech` calls `_take_speculative_relay` (consume-once, `done.wait(timeout=3.0)`, text-match + not-invalidated) to skip the post-turn LLM build → `relay timing: line_build` 1 ms vs ~500 ms. Invalidated on SPEECH_START with the other lanes; reset at capture top. Tests: `tests/test_speculative_relay.py` (6).
  - **2026-06-23 STT re-arm (`_capture_utterance` floor-downgrade):** the Smart-Turn min-speech FLOOR downgrade (short decisive callout → "incomplete" + extend) invalidated the speculative STT but, unlike the SPEECH_START and mid-pause sites, never reset the `speculative_kicked` local → Whisper re-ran COLD in the foreground after turn-close (~300 ms on the critical path). Added `speculative_kicked = False` after the floor-downgrade `_invalidate_speculative_stt()` so it re-fires on the FULL buffer during the extension. Test: `test_speculative_stt.py::test_floor_downgrade_rearms_speculative` (the invalidate is preserved — `test_floor_downgrade_invalidates_speculative` still holds).
  - **2026-06-23 boot warmup (`run()` after `audio.start()`):** SYNCHRONOUS fail-open warm of the addressing/normalize hot path — `normalize_command("push b")` + (when always-listening) `_classify_always_listening("push b", 5.0)` — so the first real callout doesn't eat the one-time ~2 s `addr+norm` cold-start (gate's first semantic/embedder inference + normalizer first-call). Logs "addressing/normalize hot path warmed". Pairs with the existing STT + LLM + reranker warmups.
...
- `push_to_talk` (HID/serial auto-PTT; `config.py` `PushToTalkConfig`) — enabled (default OFF), backend (rawhid/serial/auto/**network**), hid_vid, hid_usage_page, serial_port, baud, key, lead_ms, release_tail_ms, release_jitter_ms, heartbeat_ms, max_hold_seconds, **network_host/network_port/network_token/network_probe_timeout** (2026-07-23 two-PC hop; empty host = OFF). Env `KENNING_PTT_ENABLED` / `KENNING_PTT_SERIAL_PORT` / `KENNING_PTT_NETWORK_TOKEN`.

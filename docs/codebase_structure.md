...
>
> **Validating HEAD: `2087374`** (2026-06-12 evening — relay batch +
> rename + voice-asset reconciliation, pushed to origin/main; doc-bumps
> ride on top). **The product is now named Kenning** -- package
> `src/kenning/`, env-var prefix `KENNING_*`, runtime dirs `~/.kenning`,
> boot `python -m kenning`. GitHub repo slug / local dir
> `ultronPrototype` unchanged (real paths); the gitignored 18 GB voice
> workshop also physically stays at `ultronVoiceAudio/` (swap-back venvs
> + reference WAV — config points there, both trees protected). **Sweep
> from main: 9807 passed / 39 skipped / 0 failed, exit 0, ~186 s**
> (loaded-machine two-file-ignore recipe) + `scripts/validate_config.py`
> clean.
> **OPEN — wake-word model not deployed:** `config.yaml` points
> `wake_word.model_path` at `models/openwakeword/kenning.onnx`, which
> does NOT exist, so the detector falls back to the `hey_jarvis`
> pretrained word (graceful, warned). The 6 trained "kenning"
> candidates all undershot the production `ultron.onnx` recall; the
> spoken wake word is effectively "hey jarvis" until a candidate clears
> the bar. Detail in the 2026-06-12 rename row.
>

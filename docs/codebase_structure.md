...
|------|------|---------|-------|-------------|
| 2026-06-18 | `0163ba6` (main) | **Wake-word-required v1 + capture-stall watchdog + relay persona fix** (live testing). **WAKE WORD REQUIRED:** `addressing.follow_up_enabled` default → **False** (`config.py` + `config.yaml`). The wake-free follow-up window (acting on the addressee classifier within ~30 s of Kenning speaking) was firing on room/stream/teammate speech the user never addressed; for v1 the wake word "Ultron" is now required for **every** command (the loop sets `follow_up_until=None` after each turn → `_wait_for_wake_word`). Re-enable once wake-free confidence-gating is trusted. **CAPTURE-STALL WATCHDOG:** a USB-overrun stall after a heavy in-process LLM turn stops the mic stream's callbacks, so `get_chunk` returned `None` forever and `_wait_for_wake_word` spun idle → Ultron went permanently **deaf** (the reported freeze; no stall recovery existed). A healthy stream always delivers chunks (silence too), so consecutive `get_chunk` timeouts unambiguously mean a stall. `_wait_for_wake_word` + `_follow_up_listen` now **restart the input stream** after ~1 s of no audio (`_restart_capture_stream()`, stop+start, fail-open) so a stall right after an LLM turn recovers before the user can issue the next callout; the common no-stall case has **zero added delay**. Threshold `_CAPTURE_STALL_TIMEOUTS=2` / `_CAPTURE_STALL_SECONDS=1.0`. Unit-tested. **RELAY PERSONA:** the relay **rephrase** path called the LLM with no system prompt → inherited the base desktop "You are Kenning" persona (~4158 chars). The Ultron instruction is the user message and dominated (output stayed in character), but it wasted ~1000 tokens of the 6144 context and was a latent leak; the rephrase path now passes a focused Ultron system prompt (`_RELAY_REPHRASE_SYSTEM`), parity with the answer path. | 668 (incl. 78 addressing) | [project_prelaunch_hardening_2026_06_17.md](file:///C:/Users/alecf/.claude/projects/C--STC-ultronPrototype/memory/project_prelaunch_hardening_2026_06_17.md) |
| 2026-06-18 | `9438fc5` (main) | **Calibrated log-odds fusion for addressee confidence** (5-agent research board). Fixes the follow-up-window drop where "Ultron, show me the stop button" transcribed perfectly but scored 0.75 < the 0.80 ADDRESSED bar and was silently ignored. Two literal bugs + the architectural fix. **`zero_shot.py`** — confidence is now the model's REAL first-token P(YES) (softmax of the step-0 logits) instead of a hard-coded `0.75` constant (that constant was the literal cause; fail-open to the old constants if scores absent). **`rules.py`** — NEW `features()` graded extractor (reuses every existing regex/set); direct-address is now POSITION-AWARE and covers **"ultron"** (the rule was kenning-only and blind to the live wake word); a third-person mention ("Ultron said …", "Kenning is broken") SUPPRESSES the vocative boost and counts negative, so "Ultron, show me X" fires while "Ultron said X" does not. **`classifier.py`** — `classify()` fuses lexical features + real flan P(YES) + a recency prior in LOG-ODDS space → sigmoid → **cost-asymmetric threshold** (`tau` default 0.20; env `KENNING_ADDRESSING_TAU` to raise during a live stream). Hand-set log-odds weights (leading-wake +3.0, matcher-hit +2.2, initial-imperative +1.6, factual-q +1.0; embedded/3rd-person name −2.2, phone/interjection/narrative openers −2.0..−2.5, subject-pronoun/particle openers −1.2/−1.4). The zero-shot model is consulted **only in the undecided band** (|logit|<3), so clear commands/chatter resolve with **zero model latency** (faster than before); fails open to the legacy cascade. The blunt follow-up wake-word **bypass is removed** — the wake word is the strongest *feature*, not a hard short-circuit. Worked: "Ultron, show me the stop button" → p≈0.99 (no model call); "yeah I dunno man" / "Ultron said that" / "where's his sandbox" → NOT_ADDRESSED. Deferred (board phase-2): embeddinggemma-sidecar margin term + offline logistic fit from the addressing JSONL. | 78 addressing | [project_prelaunch_hardening_2026_06_17.md](file:///C:/Users/alecf/.claude/projects/C--STC-ultronPrototype/memory/project_prelaunch_hardening_2026_06_17.md) |
| 2026-06-18 | `a9818af` (main) | **Probe-safe import firewall + wake-word clipping + sentence-blip + mic-preprocessing fixes** (follows `75cad1c`; resolves a live-incident where the firewall silenced Ultron). **FIREWALL (`safety/import_firewall.py`)** — `find_spec` is now PROBE-SAFE: a blocked module that is NOT installed DEFERS (returns None) instead of raising, so a library's import-time optional-dep check (`importlib.util.find_spec`) is never broken by the firewall (this is the *class* fix for the pytesseract incident, where transformers→find_spec("pytesseract")→firewall raise cascaded and silenced Kokoro/Whisper/Smart-Turn). Only a blocked module that IS installed gets the hard block (installed-check consults the OTHER meta_path finders behind a thread-local reentrancy guard; submodule blocks resolve via the passed parent `path`). `assert_firewall_enforces()` now probes the blocked **and installed** `kenning.desktop` sentinel via `find_spec` (no `sys.modules` pollution) — the old absent `interception` sentinel would false-fail under the defer and brick boot (exit 4), so this lands ATOMICALLY with the find_spec change. `pytesseract` stays OFF the blocklist (the hotfix; OCR still blocked via the `kenning.desktop` prefix). Tests: probe installed sentinels + NEW regression test that an absent blocked module's `find_spec` returns None + an enforcement self-check test (401 safety tests pass). **WAKE-WORD CLIPPING (`config.py`)** — `wake_word.cold_pre_roll["ultron"]` 0.05→**0.35 s**: 50 ms was too short to cover wake-detection latency, so the first command word ("Ultron *tell* my team…" → "tell") fell in the un-captured gap (a longer manual pause was the only workaround); 0.35 s fits the 0.5 s ring and the "...tron" tail that now enters is stripped on the primary path by `command_normalizer.normalize_command`. **SENTENCE BLIPS (`tts/kokoro_engine.py`)** — an empirical per-utterance probe showed the inter-sentence gap *edges* were already clean; the real artifact is the undertrained fine-tune's noise burst at each INTERNAL sentence onset/offset, which the single `trim_and_fade` only stripped on the clip's OUTER edges → fix: run `trim_and_fade` per-chunk before joining (fail-open). Replaces the prior cosine edge-fade. **MIC PREPROCESSING (`transcription/whisper_engine.py` + `config/settings.py`)** — zero-latency `_preprocess_utterance` (DC removal → per-utterance RMS loudness-normalize to −24 dBFS with finite-check + hard +24 dB clamp → soft-limit, after the raw-peak gate, fail-open; Whisper isn't scale-invariant so quiet input drove the "Thank you" hallucination + WER variance); WIRED the decode-time confidence gates that were set in `.env` but never reached `transcribe()` (compression_ratio/log_prob/no_speech) + re-armed the temperature-fallback tuple + `prompt_reset_on_temperature` + `suppress_blank`/`without_timestamps`, all behind an `inspect`-signature filter so an older faster-whisper can't break. `PttConfig.backend` → `Literal["auto","rawhid","serial"]`. | 401 safety | [project_prelaunch_hardening_2026_06_17.md](file:///C:/Users/alecf/.claude/projects/C--STC-ultronPrototype/memory/project_prelaunch_hardening_2026_06_17.md) |

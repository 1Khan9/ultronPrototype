...
* **NEW [`src/ultron/coding/window_expand.py`](../src/ultron/coding/window_expand.py) (T5).** `WindowExpander.expand_window(lines, start, stop, max_added_lines, suffix)` extends a line range to the nearest natural break above and below. Scoring rules ported verbatim from SWE-Agent's `str_replace_editor:WindowExpander`: blank line=1, double blank=2, def/class/decorator=3, file edge=3. Direction-aware: downward expansion stops BEFORE the next def/class so the new declaration is its own block. Per-suffix `SEMANTIC_PATTERNS_BY_SUFFIX` covers Python, JS/TS, Go, Rust, Java/Kotlin/C#. Anti-shrinking guarantee: never moves boundaries inward. Returns `ExpansionResult(start, stop, expanded_lines_above, expanded_lines_below, reason_above, reason_below)`. Pure, side-effect-free, sub-millisecond on typical windows.

* **NEW [`src/ultron/coding/file_history.py`](../src/ultron/coding/file_history.py) (T20).** `FileHistory` per-session multi-file undo stack backed by `SessionRegistry`. Direct port of SWE-Agent's `_file_history` defaultdict pattern with ultron extensions: per-file cap (`max_history_per_file=10` drops oldest when full), `record_pre_edit(path, narration, origin)` captures pre-edit content + optional narration metadata, `undo_last(path)` writes back atomically via tempfile + os.replace (file-was-created case deletes; file-was-modified case restores bytes), `peek_last` / `history_for` / `all_paths` / `total_snapshots` inspection, `find_by_narration(query, n)` substring-search across recent narrations so the voice path can match "undo the change about adding the close button". Fail-open: read errors during snapshot log WARN and let the original write proceed. Round-trip across `SessionRegistry` instances tested -- partial work survives orchestrator restart.

* **NEW [`src/ultron/coding/window_state.py`](../src/ultron/coding/window_state.py) (T4).** `WindowState` persistent windowed-file state machine. Direct port of SWE-Agent's `WindowedFile` with per-session isolation. Registry keys `CURRENT_FILE` / `FIRST_LINE` (0-indexed) / `WINDOW` / `OVERLAP` match SWE-Agent's names for cross-tool legibility. `open(path, line)` sets current file + jumps to a 1-indexed line, placing it ~1/6 down the window via `GOTO_OFFSET_MULTIPLIER`. `scroll_down() / scroll_up()` advance by `window - overlap` lines (default 100 - 2 = 98) so the previous 2 lines remain visible. `view(line_numbers, status_line, pre_post_line)` renders the current window with `[File: <path> (<N> lines total)]` header and `(<X> more lines above/below)` annotations. `view_with_semantic_expansion()` integrates :class:`WindowExpander` so the rendered window covers a complete construct (function / class) instead of an arbitrary slice. Read-only state tracking -- mutating operations live in the safety + file-history layers.

* **NEW [`src/ultron/coding/edit_diagnostics.py`](../src/ultron/coding/edit_diagnostics.py) (T12).** `diagnose_edit_failure(file_text, search, replace, in_window, other_files, single_replace_required)` returns a structured `EditDiagnosticResult` with one of five `EditDiagnostic` enum values: NOT_FOUND, NOT_FOUND_IN_WINDOW, MULTIPLE_OCCURRENCES_IN_WINDOW, NO_CHANGES_MADE, AMBIGUOUS_CROSS_FILE, OK. Each non-OK diagnostic carries a rendered message matching SWE-Agent's `_NOT_FOUND` / `_NOT_FOUND_IN_WINDOW_MSG` / `_MULTIPLE_OCCURRENCES_MSG` / `_NO_CHANGES_MADE_MSG` template shapes. NOT_FOUND_IN_WINDOW lists every line where the search appears so the model can `goto` first (the highest-info-density signal you can give a stuck model). AMBIGUOUS_CROSS_FILE is the catalog's creative extension: when `other_files` is supplied and the search appears there instead, the message names each candidate file + line numbers.

* **NEW [`src/ultron/coding/lint_diff.py`](../src/ultron/coding/lint_diff.py) (T1).** `parse_flake8_output(text) -> list[LintError]` parses the canonical `file:line:col: msg` format. `shift_pre_edit_errors(pre, replacement_window, replacement_n_lines)` ports SWE-Agent's `_update_previous_errors` line-number arithmetic verbatim: errors before the window stay; errors inside the window are dropped (the edit owns them); errors after the window shift by `replacement_n_lines - window_height`. `compute_new_errors(pre, post, window, n_lines)` returns the genuinely-new errors after the shift + the SWE-Agent extra-filter that drops post-edit errors before the window (treated as pre-existing the shift missed). `format_revert_message(errors, window_applied, window_original)` renders the twin-window template verbatim (would-be window + original window + "DO NOT re-run the same failed edit" hint). `evaluate_edit_lint(pre, post, window, n_lines, window_applied, window_original)` is the end-to-end convenience that returns a `LintDiffResult(ok, new_errors, shifted_pre_errors, message)` ready for runner-side wiring. The runner integration that wires this into the existing AST listener with auto-revert via :class:`FileHistory` is the natural next-batch follow-up; the primitives ship here.

* **5 new test files (114 tests):** [`tests/coding/test_window_expand.py`](../tests/coding/test_window_expand.py) (18 -- score ordering, invalid range raises, empty file, anti-shrinking, Python def + decorator + async def expansion, JS/TS export/class, Go func, blank/file-edge fallback, max_added_lines respected, suffix override, downward-stops-before-def); [`tests/coding/test_file_history.py`](../tests/coding/test_file_history.py) (28 -- construction, record (existing/missing/empty-path/narration/origin/cap/unreadable), undo (restore/delete/no-history/atomic/empty-clear/error), peek/history/all_paths/total, find_by_narration (substring/case/recency/empty/cap), clear, factory, frozen entry, round-trip); [`tests/coding/test_window_state.py`](../tests/coding/test_window_state.py) (24 -- constants, construction, open (empty/missing/existing/with-line), close, goto without open, goto clamp, scroll down/up advance + clamp, view (status/numbers/pre_post/no-file/strip-numbers), view-with-semantic-expansion, factory); [`tests/coding/test_edit_diagnostics.py`](../tests/coding/test_edit_diagnostics.py) (18 -- find_all_in_file (empty/basic/multiline), NOT_FOUND, NOT_FOUND_IN_WINDOW (line list + re-open hint), MULTIPLE_OCCURRENCES (single-replace + replace-all bypass), NO_CHANGES_MADE, AMBIGUOUS_CROSS_FILE (only-other-files + correct lines + fallback), OK, frozen dataclasses, enum stability); [`tests/coding/test_lint_diff.py`](../tests/coding/test_lint_diff.py) (26 -- parse (basic/multiple/skip-garbage/empty), shift (before/inside/after window arithmetic + growing edit + invalid window), compute_new_errors (identical/pre-existing-after-window/before-window-filter/genuinely-new), render_window, format_revert_message (twin window + no-errors placeholder), evaluate_edit_lint (no new / introduces / fallback message), frozen dataclass).
...
The session lifts patterns from `<opencode source checkout>` (opencode reference source) and ports them to Python, ultron-shaped: typed event bus from `packages/opencode/src/bus/`, project digest template from `packages/opencode/src/session/compaction.ts`'s `SUMMARY_TEMPLATE` (re-purposed for project state instead of conversation turns), and a supervisor pattern that sits BETWEEN the routing classifier and AI coding agent dispatch -- closing the long-standing gap where Qwen had no situational awareness of `data/sandbox/` projects when routing CODE_TASK / MID_SESSION_ADJUSTMENT utterances.

* **NEW [`src/ultron/bus/`](../src/ultron/bus/) package** -- typed pub/sub with eager-subscribe race fix. [`event.py`](../src/ultron/bus/event.py): `BusEvent.define(type, schema)` + `EventPayload` envelope. [`service.py`](../src/ultron/bus/service.py): `Bus` class + module-level `publish` / `subscribe` / `subscribe_all` shortcuts + `get_bus()` singleton + `reset_bus_for_testing()`. [`events.py`](../src/ultron/bus/events.py): 17-event canonical catalog (turn lifecycle, STT/routing/gate/memory/LLM/TTS phases, coding/supervisor, safety/gaming/VRAM). Threading model: callbacks fire on the publishing thread under `RLock`; subscribers must hand off to their own queues if they need async behavior. Schema validation is best-effort + non-fatal (matches opencode's `Effect.tryPromise(...).pipe(Effect.ignore)` posture). +43 tests in [`tests/bus/`](../tests/bus/).

* **NEW [`src/ultron/coding/project_digest.py`](../src/ultron/coding/project_digest.py)** -- LLM-driven session-end digest generator (Phase A). Mirrors opencode's `SUMMARY_TEMPLATE` (Goal / Constraints / Progress {Done, In Progress, Blocked} / Key Decisions / Next Steps / Critical Context / Relevant Files) re-shaped for project state. `generate_digest(request, llm_call)` returns a `ProjectDigest` with a markdown body + parsed-section cache; fails open to a deterministic `render_template()` fallback (`source="template"`, `fallback=True`) when no LLM is wired or the LLM call raises / returns empty. Builds an update-mode prompt when a prior digest exists (anchored `<previous-summary>` block, "merge new facts, drop superseded"). Strips `\`\`\`markdown` code fences if the model wraps. +27 tests.

* **NEW [`src/ultron/coding/project_introspect.py`](../src/ultron/coding/project_introspect.py)** -- non-LLM file-tree snapshot (Phase B). `snapshot(project_path)` walks depth-limited, respects [`SKIP_DIRECTORIES`](../src/ultron/coding/project_introspect.py) (node_modules / __pycache__ / .git / .venv / etc), detects dominant language from extension counts + marker files (pyproject.toml, package.json, Cargo.toml, etc), finds entry points (manage.py / main.py / app.py / index.js / etc; root-files preferred), and -- for Python files only -- runs the existing [`ast_metadata.extract_metadata_from_path`](../src/ultron/coding/ast_metadata.py) on up to 30 files. Cached per-path with 30 s TTL. `ProjectSnapshot.render_tree_summary(max_lines)` produces a token-bounded markdown view suitable for Claude's prompt body. +25 tests.

* **NEW [`src/ultron/coding/project_index.py`](../src/ultron/coding/project_index.py)** -- Qdrant-backed digest store + semantic search (Phase B). New collection [`qdrant.collections.projects`](../src/ultron/config.py) (configurable, default `"projects"`) kept SEPARATE from conversation memory so RAG over chat history never accidentally surfaces project source. `ProjectIndex.upsert(digest)` derives a stable project_id from absolute path (UUID5 with fixed namespace, so the same project always lands on the same row across restarts). `search(query, top_k, min_score)` runs cosine-similarity over a short digest summary (Goal + Critical Context + Relevant Files concatenated, capped at 500 chars -- short embeddings retrieve better than the full body). `search_by_name(substring)` is the lexical fallback. `delete(project_id)` removes; `count()` returns total entries. Every upsert publishes [`ProjectIndexedEvent`](../src/ultron/bus/events.py) on the bus. +26 tests.

* **NEW [`src/ultron/coding/project_supervisor.py`](../src/ultron/coding/project_supervisor.py)** -- the decision layer (Phase C). `ProjectSupervisor.decide(SupervisorInputs)` returns one of four `SupervisorAction` verdicts (RESUME / EDIT / CLARIFY / NEW). Priority: (1) `has_active_task=True` + `_ADJUSTMENT_PATTERNS` match -> RESUME current session; (2) semantic top-match >= `resolve_threshold` (default 0.75) -> EDIT; (3) registry exact name/alias match -> EDIT; (4) ambiguous band `[clarify_threshold, resolve_threshold)` (default 0.55) -> CLARIFY with top-2 candidates in the question; (5) else NEW. Merges semantic candidates (from `ProjectIndex.search`) and registry candidates (from `ProjectResolver.resolve`) with `_merge_candidates` (dedupes by path, higher score wins, source labels concatenated). Every decision appended to [`logs/supervisor_decisions.jsonl`](../logs/) for offline threshold tuning. Every decision publishes [`SupervisorDecidedEvent`](../src/ultron/bus/events.py). Fail-open: supervisor exceptions return a NEW-action decision with the error in the reasoning field. +34 tests.

* **NEW [`src/ultron/coding/supervisor_dispatch.py`](../src/ultron/coding/supervisor_dispatch.py)** -- narration + enriched-dispatch coordinator (Phases D + E). `SupervisorDispatchController.dispatch(SupervisorInputs)` orchestrates: (a) call supervisor; (b) when `narrate_enabled`, speak a short TTS-safe narration via the injected `barge_in_speak` callable (delegates to the orchestrator's existing `_speak_with_barge_in_check`) and check for barge-in fire during/after playback; (c) on barge-in return `DispatchActionKind.BARGED_IN`; (d) otherwise build a `DispatchOutcome` carrying either an enriched `TaskRequest` (when `enriched_context_enabled`, prepends the prior digest + cwd file-tree snapshot + file-hints to Claude's prompt body) OR a clarification question OR a resume signal. `build_digest()` wraps `project_digest.generate_digest` with snapshot-derived language + entry-points -- used by the orchestrator's COMPLETE listener to refresh the project's digest after every Claude session. Path-safe `_speakable()` helper strips backslashes / drive letters so the narration never speaks Windows paths like "C:\Users\...". +27 tests.

* **Voice controller wiring** in [`src/ultron/coding/voice.py`](../src/ultron/coding/voice.py). `CapabilityVoiceController.__init__` gains two optional kwargs (`supervisor_dispatch`, `project_index`) -- `None` keeps the legacy `ProjectResolver` path byte-for-byte unchanged. `_handle_code_task` is intercepted at the top: when `supervisor_dispatch is not None`, route through new `_handle_code_task_via_supervisor` which:
    - Builds `SupervisorInputs` from the current runner state.
    - Dispatches via the controller.
    - Converts `DispatchOutcome` to a `VoiceResponse`:
      * `BARGED_IN` -> `VoiceResponse(text="Cancelled.", cancelled=True)`
      * `CLARIFY` -> `VoiceResponse` carrying the clarification question.
      * `RESUME_FORWARD` -> `runner.send_followup(...)` then ack.
      * `EDIT_DISPATCH` / `NEW_DISPATCH` -> `_dispatch_supervisor_task()`: mkdir cwd, touch registry, `runner.start_task(request)`, register a digest listener that fires on COMPLETE.
      * `FALLBACK` -> return `None`, drops through to legacy resolver path.
    - The COMPLETE digest listener calls `supervisor_dispatch.build_digest(...)` + `project_index.upsert(...)` so each finished session refreshes its project's digest. The `_build_supervisor_llm_call` module helper wraps `LLMEngine.generate` for the digest LLM call (with kwargs-fallback for variant signatures).

* **Orchestrator wiring** in [`src/ultron/pipeline/orchestrator.py`](../src/ultron/pipeline/orchestrator.py). `_load_coding_voice_if_enabled` consults the new [`coding.supervisor`](../src/ultron/config.py) config block; when `supervisor.enabled`, calls the new helper `_build_supervisor_stack(cfg, registry, resolver, embedder, runner)` which constructs `ProjectIndex` (if `index_enabled`), `ProjectSupervisor` (if `decide_enabled`), and `SupervisorDispatchController` (when supervisor present, with `narrate_enabled` / `enriched_context_enabled` knobs threaded through). Wires the controller's `barge_in_speak` to the existing `_speak_with_barge_in_check` method. The three components are then passed into the `CodingVoiceController` constructor. Fail-open: any construction failure logs WARN and returns three `None`s so the rest of the coding pipeline keeps legacy behavior.

* **NEW [`CodingSupervisorConfig`](../src/ultron/config.py) schema** + [config.yaml block](../config.yaml). Eleven knobs:
    ...
    - `resolve_threshold: 0.75` (EDIT cutoff)
    - `clarify_threshold: 0.55` (CLARIFY cutoff)
    ...
    All flags default OFF per the binding rollout policy. Once digests have been populated for a week (flip `digests_enabled` + `index_enabled` first, let projects accumulate), flip `decide_enabled` to start letting the supervisor route; `narrate_enabled` + `enriched_context_enabled` ship together for the full UX once decisions are vetted in the JSONL log.

* **Classifier regression fix** in [`tests/routing/test_classifier.py`](../tests/routing/test_classifier.py): 5 cases moved from `_BROWSER` to a new `_NAVIGATE_TO_SITE` test list ("open the page at example.com", "go to the page about quantum computing", "open a browser window with Google's homepage", "Can you...", "Can you open..."). 1 case ("pull up the wikipedia article on Tesla") moved to a new `_OPEN_LAST_SOURCE_AMBIGUOUS` list because the OPEN_LAST_SOURCE pattern catches it on the "article" noun. Future enhancement: extend the classifier to detect "X article on/about TOPIC" with a non-demonstrative referent and route to NAVIGATE_TO_SITE.

* **Tests added (158 net):** [`tests/bus/`](../tests/bus/) (+43: event primitives + service pub/sub + catalog), [`tests/coding/test_project_digest.py`](../tests/coding/test_project_digest.py) (+27), [`tests/coding/test_project_introspect.py`](../tests/coding/test_project_introspect.py) (+25), [`tests/coding/test_project_index.py`](../tests/coding/test_project_index.py) (+26, exercises real Qdrant + bge-small), [`tests/coding/test_project_supervisor.py`](../tests/coding/test_project_supervisor.py) (+34), [`tests/coding/test_supervisor_dispatch.py`](../tests/coding/test_supervisor_dispatch.py) (+27), plus 6 routing-test re-categorizations. **3946 -> 4104 passing.** Voice baseline unaffected (no hot-path code touched; supervisor only fires on `coding.supervisor.enabled=true` which is default OFF).

* **Roadmap items collapsed by this work:** the original opencode roadmap (event bus, provider abstraction, memory compaction, plan/build mode, sub-agent dispatch, ACP coding bridge, named profiles) was reframed during the discussion to focus on a coding-supervisor pattern -- it solves four prior items in one coherent feature (event bus = #1 standalone, compaction template = #3 applied to projects instead of conversations, plan-mode narration = #4 scoped to coding only, sub-agent intent-decision flow = #5 partial). Items #2 (provider abstraction) + #6 (ACP bridge) + #7 (named profiles) are deferred to future sessions per the reframe.

---

**2026-05-22 session D: NAVIGATE_TO_SITE intent + default-monitor preference + news-category SearxNG routing + audio overflow fix -- COMPLETE.** HEAD `9f2ac68` on `origin/main`. Tests **3946 passing / 16 skipped / 0 failed in ~70 s**. One commit on top of `385bc7c`.
...
the project-root standards doc                                          (MOST RECENT pointer + default LLM/TTS references + VRAM/test counts)
~/<ai-memory-dir>/.../memory/MEMORY.md
~/<ai-memory-dir>/.../memory/project_ultron_2026_05_20_round_8_llm_tts_swap.md
```
...
docs/codebase_structure.md             (this section + validating-HEAD bump)
the project-root standards doc                              (MOST RECENT pointer + test-count bump)
~/<ai-memory-dir>/.../memory/MEMORY.md
~/<ai-memory-dir>/.../memory/project_ultron_2026_05_20_round_7ab_memory_and_tts_chunking.md
```
...
docs/codebase_structure.md                        (this status header + per-module deltas)
the project-root standards doc                                         (most-recent-state pointer + tests count + voice baseline numbers)
```
...
the project-root standards doc                                         (most-recent-state pointer)
```
...
**2026-05-11 follow-up bug-fix pass (live-session, NEW).** Three real-session issues addressed in one pass on top of commit `431fd7b`.
...
**Tests:** 1604 → 1629 (+25 net). Files touched: 8 source + test files + this doc + the project-root standards doc. Voice baseline contract (legacy `piper_rvc` 79 ms / 7913 MB; `xtts_v3` +60 ms / +2 GB) unchanged — no hot-path edits. Config schema is backward-compatible (new field with sane default; existing `config.yaml` snapshots without `max_utterance_seconds` get the new 30 s default).

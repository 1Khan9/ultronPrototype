# Desktop automation -- OpenClaw `kenning-vision` agent + MCP tool exposure

This is the user-led setup that turns the native desktop primitives
(committed in `src/kenning/desktop/`) into a full hybrid stack with
...
- A **specialized agent (`kenning-vision`)** with a prompt tuned for
...
- **9 new MCP tools** exposed to OpenClaw agents via the existing
  stdio MCP server (`scripts/run_kenning_mcp_for_openclaw.py`):
...
- The `kenning-mcp` MCP server registered in `~/.openclaw/openclaw.json`
  (already auto-registered -- check by looking for an
  `mcp.servers."kenning-mcp"` block).
...
the `agents.list` array. The existing config has `kenning-test`
(default worker), `kenning-main` (user-facing), and `kenning-heartbeat`.
...
```powershell
openclaw agents list --json | jq '.agents[] | select(.id == "kenning-vision")'
```
...
The 9 desktop MCP tools live in `src/kenning/openclaw_bridge/mcp_tools.py`.
They register at MCP-server startup time. To verify they're exposed
to OpenClaw:

```powershell
# List Kenning's MCP tools (from OpenClaw's side)
openclaw mcp list --json
```

You should see `kenning-mcp` in the list. Then:

```powershell
# Show the tool inventory
openclaw mcp show kenning-mcp --json
```
...
```powershell
openclaw mcp unset kenning-mcp
openclaw mcp set kenning-mcp \
  --command "C:\\STC\\ultronPrototype\\.venv\\Scripts\\python.exe" \
  --args "C:\\STC\\ultronPrototype\\scripts\\run_kenning_mcp_for_openclaw.py" \
  --args "--stdio"
```

## Step 3 -- Multi-step delegation from `kenning-main`

The user-facing `kenning-main` agent should delegate complex desktop
flows to `kenning-vision`. Add this to `kenning-main`'s
...
delegate to the kenning-vision agent rather than calling MCP tools
directly. Single-step requests ("open YouTube on monitor 2",
"explain what I'm looking at") are handled by Kenning's native
voice path -- do not delegate those.
```

OpenClaw's agent-to-agent handoff happens via `openclaw agent --json
--agent-id kenning-vision --message "<task description + observed
...
C:\STC\ultronPrototype\.venv\Scripts\python.exe scripts\run_kenning_mcp_for_openclaw.py --list-tools
```

You should see the 14 tools listed (5 heartbeat/coding + 9 desktop).

Then from inside OpenClaw, drive `kenning-vision` with a small task:

```powershell
openclaw agent --agent-id kenning-vision --message "Get the screen context, then tell me which monitor has Discord on it." --json
```

Expected: `kenning-vision` calls `get_screen_context`, parses the
...
**`kenning-vision` doesn't appear after Gateway restart.** Verify
...
add a `set_vlm()` call to `scripts/run_kenning_mcp_for_openclaw.py`
(out of scope for v1).

## Security posture

Every MCP-tool call ultimately routes through the runtime tool-call
validator under `src/kenning/safety/` (the 141-rule, 19-category
...
- "open YouTube on my second monitor" -> Kenning-side voice path
  (`SCREEN_CONTEXT_QUERY` / `APP_LAUNCH` intent), native Chrome
  launch + monitor placement. No OpenClaw round-trip.
- "explain what I'm looking at" -> Kenning-side voice path,
  `SCREEN_CONTEXT_QUERY` builds a screen-context snapshot
  (optionally with VLM), injects into the LLM, Kenning answers.
- "log into my bank and download my statements" -> `kenning-main`
  delegates to `kenning-vision`, which plans + observes + acts

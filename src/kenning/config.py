...
like the Brave API key — declare ``brave_api_key_env: "KENNING_BRAVE_API_KEY"``
...
def resolve_path(value: str | Path) -> Path:
    ...
    config values like ``~/.kenning/sandbox`` work -- previously a
...
class AudioConfig(_Strict):
    ...
    # the wake-word "Kenning" tail does not bleed into Whisper as a
...
class STTConfig(_Strict):
    ...
    # isolated venv at ``kenningVoiceAudio/.venv-parakeet/`` to
    ...
    # ``.venv-xtts`` + ``kenningVoiceAudio/scripts/xtts_server.py``.
    #
    # To set up: run ``python -m venv kenningVoiceAudio/.venv-parakeet``
...
class LLMServerConfig(_Strict):
    ...
    ``scripts/start_llamacpp_server.py``. Both Kenning's voice
...
class LLMCompressionConfig(_Strict):
    ...
    hook is plumbed in via :class:`kenning.llm.compression.Compressor`
...
class LLMPersonaConfig(_Strict):
    ...
    Phase 1 of the OpenClaw integration migrated Kenning's persona
    out of ``config.yaml:llm.system_prompt`` and into the shared
    workspace files (``SOUL.md``, ``IDENTITY.md``, ``USER.md``,
    ``AGENTS.md``). Both Kenning's voice pipeline and OpenClaw read
...
LLM_PRESETS: dict[str, dict[str, Any]] = {
    ...
    # The voice path keeps Kenning's persona via SOUL.md system prompt;
    # the model's abliterated nature removes content-level refusals while
    # the runtime tool-call validator (src/kenning/safety/) gates the
...
class LLMConfig(_Strict):
    ...
    #                         tool-call validator in src/kenning/safety/.
    ...
    #              :mod:`kenning.llm.draft_model`. Theoretical 30-50%
    ...
    # history -- kenning's voice path rarely sees them), last-N OFF
...
class BackgroundSummarizerConfig(_Strict):
    ...
    :class:`kenning.memory.background_summarizer.BackgroundSummarizer.maybe_summarize`
...
class MemoryHistoryCompressionConfig(_Strict):
    ...
    :meth:`kenning.memory.background_summarizer.BackgroundSummarizer.compress_history_for_llm`
...
class WebSearchQueryReformulationConfig(_Strict):
    ...
    queries that :class:`~kenning.web_search.search.WebSearchExecutor`
    merges into its existing query list (deduped + URL-merged through
    the same provider chain + cache; a hard
    :data:`~kenning.web_search.query_rewrite.MAX_TOTAL_QUERIES` ceiling
    bounds the fan-out).
...
class WebSearchConfig(_Strict):
    enabled: bool = True
    brave_api_key_env: str = "KENNING_BRAVE_API_KEY"
...
class AddressingConfig(_Strict):
    ...
    # third-person narration about Kenning was getting routed to
...
class CodingCanonicalMonitorConfig(_Strict):
    ...
    :mod:`kenning.coding.canonical_monitor`; they cover the standard
...
class CodingMCPConfig(_Strict):
    ...
    server_name: str = "kenning_coding"
...
class CodingAstMetadataConfig(_Strict):
    ...
    :mod:`kenning.coding.ast_metadata` and emits an audit-log row
...
class CodingAiCommentWatcherConfig(_Strict):
    ...
    :class:`kenning.coding.ai_comment_watcher.AICommentWatcher` rooted
...
class CodingDialogAutoHandlerConfig(_Strict):
    ...
    :data:`kenning.bus.events.DialogAppearedEvent` for the lifetime of
...
class CodingPreEditSnapshotConfig(_Strict):
    ...
    :class:`kenning.coding.file_history.FileHistory` BEFORE the CLI's
    ...
    * **SWE-Agent T14 edit_recovery** --
      :func:`kenning.coding.edit_recovery.run_edit_with_recovery` can
...
class CodingArchitectConfig(_Strict):
    ...
    :class:`kenning.coding.architect_supervisor.ArchitectSupervisor` and
...
class CodingRepoMapConfig(_Strict):
    ...
    :class:`kenning.coding.repo_map.RepoMapProviderCache` and wires it
    ...
    cache_dir: str = "data/.kenning_repomap_cache"
...
class CodingConfig(_Strict):
    ...
    # override via config.yaml or the KENNING_CLAUDE_CLI env var.
    ...
    # OUTSIDE the kenning repo tree. The coding CLI walks UP from the task
    ...
    # when git-initialised; outside dirs are clean). ``~/.kenning/`` is
    ...
    sandbox_root: str = "~/.kenning/sandbox"
...
class XttsV3Config(_Strict):
    """Configuration for the XTTS v2 + v3 Kenning filter TTS engine.
    ...
    # we hand it a 3-minute Kenning_vocals_mono_v1.wav, only ~6 s
...
class KokoroConfig(_Strict):
    ...
    # Voice tensor name. Production target is a fine-tuned Kenning
    ...
    # partial Kenning fine-tune (Stage 1 complete + Stage 2 epoch 0
    ...
    # comparing windows 3/5/7/9 on the 16-sentence Kenning test set).
...
class OutputWatchConfig(_Strict):
    ...
    :class:`kenning.audio.output_quality.OutputQualityWatcher`, which
...
class TTSConfig(_Strict):
    ...
    # Expected output sample rate for speculative open. The Kenning RVC
...
class AmbiguityBandClarificationConfig(_Strict):
    ...
    The pure predicate :func:`kenning.openclaw_routing.ambiguity.should_clarify`
...
class OpenClawBridgeConfig(_Strict):
    ...
    - Kenning starts up (registers Kenning MCP with the Gateway).
...
class OpenClawConfig(_Strict):
    ...
    # Must name a REAL agent id in ~/.openclaw/openclaw.json (the
    # kenning-main user-facing persona); every dispatcher call routes
    # through this id.
    required_agent_id: str = "kenning-main"
...
class NotificationsConfig(_Strict):
    """Phase 4 — proactive notifications from Kenning to remote channels.
...
class BrowserConfig(_Strict):
    ...
    Kenning's wrapper (:class:`BrowserTool`) just routes intent to
    ...
    Kenning's voice (precise, weighted, no filler)."""
...
class MediaGenerationConfig(_Strict):
    ...
    **Provider policy (project-wide):** Kenning only accepts
    ...
    Kenning just routes the intent. See
...
class HeartbeatConfig(_Strict):
    """Phase 5 — heartbeat alert persistence + Kenning-side query.

    The OpenClaw-side heartbeat agent (configured separately in
    ``~/.openclaw/openclaw.json`` under ``agents[].heartbeat``) raises
    alerts that Kenning records here so a voice query like "what alerts
...
class ErrorPhrasesConfig(_Strict):
    """User-facing voice messages for dependency failures, in Kenning's voice.
...
class ClickPreviewConfig(_Strict):
    ...
    :mod:`kenning.desktop.click_preview` gate: a red crosshair is drawn
...
class BrowserUseConfig(_Strict):
    ...
    extraction in :func:`kenning.desktop.uia.extract_browser_content`.
    ...
    * ``BROWSER_USE_SESSION`` env var is SCRUBBED on every subprocess
    ...
    Attributes:
        ...
        screen_context_fallback_enabled: when True, the new tier
            slots into :func:`kenning.desktop.screen_context.build_screen_context`
...
class SafetyConfig(_Strict):
    ...
    # matcher in kenning.safety.intent. It NEVER overrides a BLOCK_HARD and only
...
class SkillsConfig(_Strict):
    ...
    user_dirname: str = ".kenning/skills"
    project_dirname: str = ".kenning/skills"
...
class DeepResearchConfig(_Strict):
    ...
    An EXPLICIT-opt-in multi-step research mode over kenning's FREE
    local-first search ladder. When the user asks to "research X in depth"
    / "do a deep dive on X" (matched by
    :func:`kenning.web_search.deep_research.match_deep_research`), the
    orchestrator runs a
    :class:`~kenning.web_search.deep_research.DeepResearchLoop` (subclass of
    the catalog-11 :class:`~kenning.agent_loop.base.AgentLoop`): decompose ->
    search each sub-question via the normal
    :class:`~kenning.web_search.search.WebSearchExecutor` -> identify gaps ->
...
class EvolutionConfig(_Strict):
    ...
    Kenning observes its own turns, mints success/failure *capsules*, and --
    ...
    :class:`~kenning.evolution.evolution_loop.EvolutionLoop` (a subclass of the
    catalog-11 :class:`~kenning.agent_loop.base.AgentLoop`): pre-flight gate
    ...
    generated code, NEVER ``src/kenning/``, NEVER a Category-K surface; the
...
class HooksConfig(_Strict):
    ...
    scripts discovered under ``~/.kenning/hooks/<HookName>`` (global) and
    ``<project>/.kenning/hooks/<HookName>`` (project): a ``TaskStart`` hook may
...
class McpServerSpec(_Strict):
    """One external MCP server the operator wants Kenning to manage (T22).
...
class SpotifyConfig(_Strict):
    ...
    (``credentials_path``, default ``~/.kenning/spotify.json``) holding
    ...
    enabled: bool = True
    credentials_path: str = "~/.kenning/spotify.json"
...
class RelaySpeechConfig(_Strict):
    ...
    "Kenning, tell my teammates to smoke mid" is an instruction to DELIVER
    a spoken line into the game voice chat, not a conversational prompt.
    When enabled, a strict matcher
    (:func:`kenning.audio.relay_speech.match_relay_command`) intercepts the
    ...
    B-bus as the user's microphone, so teammates hear Kenning through the
    mic channel.
    ...
    # strict relay match is definitionally addressed to Kenning).
    follow_up_seconds: float = Field(default=120.0, ge=0.0, le=600.0)
    # User-curated roast lines for "Kenning, roast my team" -- one line
    # per row, # comments ignored, spoken VERBATIM (never LLM-authored).
    # The file is seeded with a default line on first use; edit it any
    # time, it is re-read per roast.
    roast_lines_path: str = "data/relay_roasts.txt"
    # Fun-fact corpus for "Kenning, tell my team a fun fact" -- one fact
    # per row, # comments ignored, spoken VERBATIM. Ships in the repo;
    # re-read per request so edits/additions apply live.
...
class KenningConfig(_Strict):
    ...
    # in ``kenning.desktop.uia.extract_browser_content``. Default ON
    ...
    # When enabled, walks ``skills/`` (public), ``~/.kenning/skills/`` (user),
    # ``<project>/.kenning/skills/`` (project), and injects matching skill
    ...
    # TaskComplete fire user scripts under ~/.kenning/hooks/. Zero-cost when no
...
def load_config(
    ...
    with all defaults, pass ``KenningConfig()`` directly via tests; the
    production path REQUIRES a real file.
    """
    global _CONFIG_INSTANCE, _CONFIG_PATH
    if path is None:
        env_override = os.environ.get("KENNING_CONFIG_PATH")
        path = Path(env_override) if env_override else DEFAULT_CONFIG_PATH

    # Late import: kenning.errors imports nothing from this module, but we
    ...
    if not path.is_file():
        raise ConfigurationError(
            f"config.yaml not found at {path}. Set KENNING_CONFIG_PATH or "
    ...
    # ``KENNING_LLM_PRESET=qwen3.5-4b python -m kenning`` picks the preset
...
# exhaustive -- the log also dumps every KENNING_* env var present, so
...
_ENV_OVERRIDE_NOTES: dict[str, str] = {
    "KENNING_LLM_PRESET": "llm.preset",
    "KENNING_LLM_MODEL_PATH": (
...
def log_effective_config(
    cfg: Optional["KenningConfig"] = None,
    *,
    ...
    Surfaces the most-frequently-confused settings:
      * Every ``KENNING_*`` environment variable that's set (values
    ...
    """
    if logger is None:
        logger = logging.getLogger("kenning.config.effective")
    if env is None:
        env = os.environ
    try:
    ...
    # --- KENNING_* env vars ---
    kenning_keys = sorted(k for k in env.keys() if k.startswith("KENNING_"))
    if kenning_keys:
        logger.info("effective-config: %d KENNING_* env var(s) set:", len(kenning_keys))
        for key in kenning_keys:
        ...
        logger.info("effective-config: no KENNING_* env vars set")

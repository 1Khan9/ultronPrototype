...
class Knob:
    ...
    Attributes:
        ...
        restart: True when the value is read at construction time and
            only applies on the next Kenning start. (No knob in the
        ...
        choices_provider: name of a dynamic choices source resolved at
            panel build time (``"output_devices"`` enumerates PortAudio
            output devices). Used when the option set depends on the
            machine, not the config schema; static ``choices`` take
...
def output_device_names() -> tuple[str, ...]:
    """Enumerate PortAudio OUTPUT device names on this machine.

    Used by the ``output_devices`` choices provider (the relay
    output-device dropdown). Lazy sounddevice import; enumeration never
    opens a device. Fail-open: any backend problem returns () and the
    caller keeps the current config value as the only choice.
    """
    try:
        ...
        for device in sd.query_devices():
            ...
            if name and name not in names:
                names.append(name)
        return tuple(names)
    except Exception:  # noqa: BLE001 - panel must open regardless
        return ()


def resolve_choices(knob: Knob, current: Any) -> tuple[str, ...]:
    """Final dropdown options for a ``choice`` knob.

    Static ``choices`` win when declared; otherwise the knob's
    ...
    return provided or ((current_text,) if current_text else ())

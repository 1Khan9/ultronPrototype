"""The Kenning control panel (tkinter, dark theme).

Runs in its OWN process (``python -m kenning.settings_gui``). Layout:

    +--------------------------------------------------------------+
    |  KENNING // CONTROL PANEL                     status text     |
    |  +--------------------+  +------------------------------+    |
    |  | settings cards     |  |  LIVE LOG  [filter] [pause]   |    |
    |  | (scrollable, 2-col)|  |  streaming logs/kenning.log    |    |
    |  +--------------------+  +------------------------------+    |
    |  [ pending-changes status ]      [ APPLY UPDATE ] [ CLOSE ]   |
    +--------------------------------------------------------------+

Update: patches config.yaml (comment-preserving), then touches the
reload signal so the RUNNING Kenning hot-applies every live-readable
...
# ---------------------------------------------------------------------------
# Palette -- near-black with Kenning crimson accents.
# ---------------------------------------------------------------------------
...
class ControlPanel:
    ...
    def __init__(self, config_path: Path, log_path: Path,
        ...
        self.root = tk.Tk()
        self.root.title("KENNING // CONTROL PANEL")
    ...
    def _build(self) -> None:
        ...
        ttk.Label(header, text="KENNING", style="Title.TLabel").pack(side="left")
    ...
    def _make_widget(self, parent: tk.Frame, knob: Knob,
        ...
        if knob.kind == "choice":
            ...
            return var, ttk.Combobox(
                ...
                state="readonly", width=width, font=UI_SMALL)
        if knob.kind == "csv":
            var = tk.StringVar(
...
def main() -> int:
    """Entry point for ``python -m kenning.settings_gui``."""
    from kenning.config import LOGS_DIR, PROJECT_ROOT

    panel = ControlPanel(
        config_path=Path(PROJECT_ROOT) / "config.yaml",
        log_path=Path(LOGS_DIR) / "kenning.log",

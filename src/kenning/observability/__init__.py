...
Public surface for ultron's internal aggregate-only metrics (which
...
The package ships :mod:`ultron.observability.private_telemetry`
...
is in place for a future opt-in (``ULTRON_TELEMETRY=opt-in``); the
default is fail-private (``ULTRON_DISABLE_TELEMETRY=1`` semantics
